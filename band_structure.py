...
from numpy import zeros, dot, resize, amax
from matplotlib import pyplot as plt
from gpaw import GPAW

calc = GPAW(str(argv[1]))
...
class BandStructureInterface(object):
    ...
    def initialize_band_structure(self):
        ...
        self.bs_matrix = [[BandElementInterface(band, k, 0) for k in range(self.nkpts)]
                          for band in range(self.nbands)]
        for band in range(self.nbands):
            for k in range(self.nkpts):
                self.bs_matrix[band][k].evalue = self.calc.get_eigenvalues(k)[band]
        ...
        for k in range(self.nkpts-1):
            ...
            for band in range(self.nbands):
                for n_prim in range(self.nbands):
                    percentage = k/float(self.nkpts)*100 + 1/float(self.nkpts)/self.nbands*band*100
                        #For LCAO, the program runs through the ns, given k (viceversa for RS)
                    print("Currently overlapping n: " + str(band) + ". k: " +str(k) +". n': " +
                          str(n_prim) + "\t" +str(round(percentage, 3)) + " percent completed.")
                    self.bs_matrix[band][k].overlaps[n_prim] = overlaps[band][n_prim]#mod
                self.bs_matrix[band][k].overlaps = sorted(self.bs_matrix[band][k].overlaps.items(),
                                                          key=lambda x: -x[1])
        ...
        for k in range(self.nkpts-1):
            ...
            for band in range(self.nbands):
                for n_prim in range(self.nbands):
                    percentage = k/float(self.nkpts)*100 + 1/float(self.nkpts)/self.nbands*band*100
                        #For LCAO, the program runs through the ns, given k (viceversa for RS)
                    print("Currently overlapping n: " + str(band) + ". k: " +str(k) +".band': " +
                          str(n_prim) + "\t" +str(round(percentage, 3)) + " percent completed.")
                    self.bs_matrix[band][k].overlaps[n_prim] = overlaps[band][n_prim]#mod
                self.bs_matrix[band][k].overlaps = sorted(self.bs_matrix[band][k].overlaps.items(),
                                                          key=lambda x: -x[1])
    ...
    def get_overlap(self, kpt1, kpt2):
        ...
        for i in range(self.nbands):
            ...
            wf_nk1 = calc.get_pseudo_wave_function(i, kpt1)
                #Normalize wf_nk1"
            total_band = (wf_nk1.conjugate()*wf_nk1).sum()
            for j in range(self.nbands):
                    #Obtain the second wave function wf_nk2"
                wf_nk2 = calc.get_pseudo_wave_function(j, kpt2)
                    #Get the overlap"
                overlp = abs(((wf_nk1.conjugate()*wf_nk2).sum())/total_band)
    ...
    def initialize_rs_overlaps(self):
        ...
        for band in range(self.nbands):
            for k in range(self.nkpts - 1):
                for n_prim in range(self.nbands):
                    percentage = band/float(self.nbands)*100 + \
                                    1/float(self.nbands)/self.nkpts*k*100
                    print("Currently overlapping band: " + str(band) +
                          ". k: " +str(k) +". band': " +
                          str(n_prim) + "\t" +str(round(percentage, 3)) + " percent completed.")
                    self.bs_matrix[band][k].overlaps[n_prim] = abs(
                        (self.calc.get_pseudo_wave_function(band, k).conj() *
                         self.calc.get_pseudo_wave_function(n_prim, k+1)).sum() /
                        (self.calc.get_pseudo_wave_function(band, k).conj() *
                         self.calc.get_pseudo_wave_function(band, k)).sum())
                self.bs_matrix[band][k].overlaps = sorted(
                    self.bs_matrix[band][k].overlaps.items(), key=lambda x: -x[1])
                        #Cloning the dictionary into a overlap-ordered list [(nband, overlap),...}
    ...
    def match_bands(self):
        ...
        while not done:
            for k in range(self.nkpts - 2):
                for band in range(self.nbands):
                    current_element = self.bs_matrix[band][k]
                    ...
                    print("Currently matching n: " + str(band) + ". k: " +str(k))
                    if next_match.prev is None:   #If the best overlap candidate is unmatched
                        ...
                        print("Normally matched n: " + str(band) + ". k: " +str(k) + "with n': " +
                              str(n_next) + " " + str(k+1))
                    ...
                    #One of them should be lower, then clear the lower
                        ...
                        if current_element.overlaps[
                            ...
                            print("Conflict n: " + str(band) + ". k: " +str(k))
                            best_overlap_index += 1
                            if k == self.nkpts - 3 and band == self.nbands - 1:
                                done = True
                                print("Ended loop")
                        else:
                            print("Unmatching n: " + str(band) + ". k: " +str(k) +"with . n: ")
    ...
    def get_lcao_overlap(self, kpt_1, kpt_2):
        """Calculate LCAO overlaps for kpts kpt_1 and kpt_2"""
        calc.set_positions(calc.get_atoms())
        kpt1 = calc.wfs.kpt_u[kpt_1]
        kpt2 = calc.wfs.kpt_u[kpt_2]
    ...
    def get_paw_overlap(self, kpt_1, kpt_2):
        """Calculate PAW Corrections to the overlap qvnm matrix"""
        proj_ani = self.get_proj_ani(kpt_1)
        # PAW corrections from other spin channel
        proj_ami = self.get_proj_ani(kpt_2)
        nbands = calc.get_number_of_bands()
        dtype = calc.wfs.dtype
    ...
    def get_pw_overlap(self, kpt_1, kpt_2):
        """Calculate PW overlaps for kpts kpt_1 and kpt_2"""
        calc.set_positions(calc.get_atoms())
        kpt1 = calc.wfs.kpt_u[kpt_1]
        kpt2 = calc.wfs.kpt_u[kpt_2]
        nbands = calc.get_number_of_bands()
        ...
        ws_1 = kpt1.psit_nG[:]
        ws_2 = kpt2.psit_nG[:].transpose().conj()
        if ws_1.shape[1] != ws_2.shape[0]:
            dim_min = min(ws_1.shape[1], ws_2.shape[0])
            if ws_1.shape[1] < ws_2.shape[0]:
                ws_2 = resize(ws_2, (dim_min, ws_2.shape[1]))
            if ws_1.shape[1] > ws_2.shape[0]:
                ws_1 = resize(ws_1, (ws_1.shape[0], dim_min))
            pw_overlap = abs(dot(ws_1, ws_2))
        norm = abs(dot(ws_1, ws_1.conjugate().transpose())).diagonal()
        pw_overlap /= norm
        for i in range(self.nbands):
            if amax(pw_overlap[i]) < 0.6:
                self.counter += 1
                return self.get_overlap(kpt_1, kpt_2)
        return pw_overlap
